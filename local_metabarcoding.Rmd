...
tax <- t(sapply(ids, function(x) {
  ...
  mutate_all(str_replace,pattern="(?:.(?!_))+$", replacement="") %>%
  magrittr::set_rownames(getSequences(seqtab_final)) %>%
  seqateurs::na_to_unclassified(rownames=TRUE) %>% #Propagate high order ranks to unassigned ASV'
  magrittr::set_rownames(getSequences(seqtab_final)) %>%
  as.matrix()
...
tax <- assignTaxonomy(seqtab_final, "reference/rdp_genus.fa.gz", multithread=TRUE, minBoot=60, outputBootstraps=FALSE)
colnames(tax) <- c("Root", "Phylum", "Class", "Order", "Family", "Genus")

##add species to taxtable using exact matching
tax_plus <- addSpecies(tax, "reference/rdp_species.fa.gz", allowMultiple=TRUE)
...
saveRDS(tax, "output/rds/tax_rdp.rds") 

tax <- readRDS("output/rds/tax_rdp.rds")
...
# Optional - Compare to a BLAST top hit  {-}
...
out <- blast_top_hit(query=seqs, db="reference/heirarchial.fa.gz", threshold=60 )
...
export <- speedyseq::psmelt(ps) %>%
  filter(Abundance > 0)
write.csv(export, file = "output/results/unfiltered/rawdata.csv")
