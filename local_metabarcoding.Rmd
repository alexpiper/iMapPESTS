...
for (i in 1:length(runs)){
  ...
  filtered_out[[i]] <- filterAndTrim(fwd = file.path(path, fastqFs), filt = file.path(filtpath, fastqFs),
                                      rev = file.path(path, fastqRs), filt.rev = file.path(filtpath, fastqRs),
                                      maxEE = maxEE, truncLen = 0, minLen = minlength, maxLen = maxlength ,
                                      rm.lowcomplex = rm.lowcomplex, rm.phix = TRUE, 
                                      matchIDs = TRUE, id.sep = "\\s",
                                      multithread = TRUE, compress = TRUE, verbose = TRUE)
...
codon_filt <- codon_filter(seqs, genetic_code = 'SGC4') # Default is invertebrate mitochondial code
